...
**S27 — No blocking calls on non-blocking/event-loop threads.** `tier: A`, `metastable_role: amplifier`. A blocking call (JDBC, `Thread.sleep`, a blocking `HttpClient`) inside a Reactor operator, WebFlux handler, Netty channel handler, or Akka Streams stage occupies a thread meant to service many concurrent requests; one slow call starves the entire event loop rather than one request. Detector: `kind: regex`, anchored on blocking-call idioms, with `present_within` requiring a Reactor/Netty/WebFlux/Akka import or type in the enclosing file (a blocking call in a plain servlet controller is not a violation of this pattern — that thread model expects blocking calls).

**S28 — Bounded, timed lock/wait acquisition.** `tier: A`, `metastable_role: sustaining`. `synchronized` or `Lock.lock()`/`Condition.await()` with no timeout variant means a stalled holder blocks every other thread indefinitely — a convoy that compounds under load instead of shedding it. Detector: `kind: regex`, flags `synchronized` blocks and untimed `lock()`/`await()` calls, `absent_within` checks for `tryLock(`, `lockInterruptibly(` is not itself sufficient (no timeout), only the timed overloads (`tryLock(long, TimeUnit)`, `await(long, TimeUnit)`) suppress the hit.
...
| S19 No error swallowing | empty/log-only `catch` in Kafka consumer, Akka actor `receive`, JMS `onMessage` |
...
- `s29_n_plus_one` is a new handler: given a method body, it looks for a loop over a collection-typed variable followed, within the loop body, by a call to a getter (`getXxx()`/property accessor) on the loop variable that isn't guarded by an eager-fetch hint (`@EntityGraph`, `JOIN FETCH` in a co-located query string, `Hibernate.initialize(`) earlier in the same method. This is a heuristic, not a real dataflow analysis — false positives are expected and constrained the same way S07/S10 are: a negative sample for "loop touches an already-eagerly-fetched association" is required.
...
1. Run the batch's detectors (via `uv run scripts/signals.py --repo <path>`) against 3–5 real, public Java repositories representative of that batch — e.g. a Spring Boot reference/sample app, a Kafka consumer library, a project using resilience4j — chosen for the Spring/Hibernate/Kafka and resilience4j/gRPC batches; the JDK/core batch can calibrate against any idiomatic Spring/plain-Java service, since it has no framework-specific risk.
