...
- `SLEEP_RES["java"]` matches any sleep-named call: `Thread.sleep(`, `TimeUnit.SECONDS.sleep(`, a bare inherited `sleep(`, and helpers such as `quietlySleep(`/`sleepUninterruptibly(`. The first argument is the wait. Accessors (`set`/`get`/`is`/`has` followed by a capital letter, e.g. `setMaxSleepMs(`) configure or read a wait and are excluded. For Java only, `s02_backoff` also requires the sleep to sit inside an enclosing `for`/`while`/`do` loop, found by walking back up the enclosing braces to the class header. A sleep with no loop around it is simulated latency or a one-off pause, even when `@Retry` is a few lines away. TS/Python keep the shared window-only rule (AC-16).
...
- `s18_fail_fast` gains Java-specific vocabularies, selected by `_lang_key`:
  - `FUNC_JAVA`: member boundaries are `METHOD_JAVA` declarations, plus any line that opens a field, nested type or member with an access or `static` modifier.
  - `VALIDATION_JAVA`: `throw new IllegalArgumentException`, `Objects.requireNonNull`, `Preconditions.check*`, `Assert.*`, and a `BAD_REQUEST` `ResponseStatusException`. A throw on a switch label or within two lines of a `catch` is not validation.
  - `EXTERNAL_CALL_JAVA`: Java's own case-sensitive list (`send`, `exchange`, `retrieve`, `get/postFor…`, JDBC `execute…`/`query…`, `execute` on a non-executor receiver, `generateContent`), not the shared set plus extras. The shared list matches case-insensitively, so its `fetch` prefix hits `fetchSize`/`FetchType`, `.query(` hits `.Query(`, and words inside strings (`got`) count. It also treats `.invoke(` (reflection) and `executor.execute(` (a task hand-off) as calls.
  - Response-derived exemption: a check after the call is response handling, not late validation, when it names the variable the call line assigned (or one later assigned from it) or reads `getBody()`/`body()`/`getStatusCode()`/`statusCode()`.
  The shared `VALIDATION`/`EXTERNAL_CALL` regexes are not edited, because that would change TS/Python behaviour (AC-16).
- `s10_retry_layers` gets `RETRY_LAYERS["java"]`, which counts retry *mechanisms*: a hand-written loop, Spring Retry, resilience4j and Failsafe. `maxAttempts` is not counted separately, because it configures one of those mechanisms rather than adding a layer. For Java the handler counts layers **per method**, not per file. Each retry mechanism found is bound to its owning declaration by `METHOD_JAVA`: an annotation binds forward to the next declaration, and a statement binds back to the one above it. So `@Retryable` on one method and `@Retry` on another are one layer each. It also skips `@Configuration`/`@AutoConfiguration` classes. They define retry beans and never call through them, so two bean definitions are not two stacked layers.
- `s07_checkpoint` uses Java-only paging vocabularies (`PAGING_JAVA`, `PAGE_ADVANCE_JAVA`, `PERSIST_JAVA`). `Iterator.hasNext()`/`hasMoreElements()` walk an in-memory collection, and a Java `offset` is as often a byte or Kafka record offset as a page position. So a more-pages test counts only as a flag or on a page/response/slice receiver, and an offset advance counts only when it steps by a page or batch size. A progress setter (`setLastOffset(…)`) counts as persisting the position, and a request setter (`setPageToken(…)`) does not.
...
**Confidence rests on the calibration outcome.** `medium` is a ceiling, not a default. The final cross-batch sweep (`docs/calibration/java.md`, Batch 6) swept every Java pattern over all 18 calibration repositories. A detector kept `medium` only if that sweep showed at least one inspected true positive and every false positive was fixed with a negative sample. Four detectors met that bar and stay `medium`: `S01-java-httpclient-no-connect-timeout`, `S01-java-httprequest-no-timeout`, `S19-java-empty-catch` and `S19-java-catch-only-comment`. Four were demoted to `low`:
- `S02-java-backoff`: a poll-loop pause cannot be told apart from a retry wait without touching the shared handler.
- `S03-java-429-ignores-retry-after`: it has no real-world true positive.
- `S14-java-unbounded-blocking-queue` and `S14-java-executors-unbounded-queue`: queues and executors that are bounded by construction are invisible to a regex.
