...
- [ ] **Step 1:** Write table-driven failing tests for the pure part of spec §10: `parse_remote`, `detect_provider`, `encode_path` (after `ref_path`), `LinkContext.code` for range, single line and whole file, `?plain=1`, `LinkContext.commit`, template fill in a single pass, the whole-file template cut, and `parse_lines`. Include the cases `"C:\\repos\\x"`, `"https://github.com/"`, `"https://github.com:bad/x"`, `"../x"`, a base with braces, `"../src/x.ts"` → `src/x.ts`, and `" 16 - 28 "` / `"L16"` / `"16–28"` / `"28-16"`.
- [ ] **Step 2:** Run `pytest tests/test_links.py -q` and confirm the failures.
- [ ] **Step 3:** Implement:
...
- [ ] **Step 3:** Implement `LinkConfig`, `config_from_profile(profile)` and
...
- [ ] **Step 4:** Run `pytest tests/test_links.py -q` and confirm it passes.
- [ ] **Step 5:** Commit: `Resolve the link base, stale files, commit ids and push state for report links`.
...
- [ ] **Step 1:** Write the failing pipeline tests from spec §10. On `linked_copy`, write `_valid_finding` with `_write_finding`, then `_validate` and render. Assert that:
...
- [ ] **Step 2:** Implement in `render_json`. **Step 3:** Run the full suite.
- [ ] **Step 4:** Commit: `Carry permalinks in report.json`.
...
- [ ] **Step 1:** Write `test_sample_report_links_its_refs`. It fails against the current sample.
- [ ] **Step 2:** Update the generator and run `uv run scripts/gen_sample_report.py`. Review the diff: only the ref formatting, the header comment and the scan date may change.
- [ ] **Step 3:** Update the docs, the example profile and the version, and add the CHANGELOG entry.
- [ ] **Step 4:** Verify:
