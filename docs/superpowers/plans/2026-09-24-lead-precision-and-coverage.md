...
Requirements and acceptance criteria AC-1…AC-19 are in the GitHub issue
"Trustworthy leads and visible coverage".

**Tech stack:** Python ≥ 3.11 stdlib + `pyyaml` + `lizard`, pytest, `uv run`.
No new dependencies.

**Branch:** `claude/plugin-report-review-blind-spots-g8iuhy` for the spec
and plan; implementation on a feature branch, one commit per task. Tick the
...
**Files:** `scripts/signals.py` (`classify_commit` moves here from `bundle.py`;
`bundle.py` imports it), `scripts/bundle.py` (label rendering),
...
```python
import pytest
from signals import classify_commit
...
### Task 8: `high` needs a fix commit; commit subjects in the report

Satisfies: AC-5, AC-3.

**Files:** `scripts/validate.py`, `scripts/report.py`,
`agents/thunderstruck-investigator.md`, `skills/thunderstruck-scan/SKILL.md`
(if it restates the rule), `scripts/gen_sample_report.py`, `tests/test_pipeline.py`.
...
- [ ] **Failing tests** in `tests/test_pipeline.py`:
  ...
  - the report for a valid finding contains the commit's subject in quotes
    and `(fix)`.
- [ ] **Implement.** `Validator._subject(sha)` is cached and uses
  `git log -1 --format=%s`. In `check_finding`, when `conf == "high"`,
  require `any(classify_commit(self._subject(sha)) == "fix" …)` over commit
  refs that already passed `check_commits_touch`. `report.py collect()`
  attaches `subject` and `kind` to each commit evidence item, and rendering
  follows spec §2.3.
- [ ] **Sample report:** FR-003's only commit is the one that introduced the
  comment. Set it to `medium` in `gen_sample_report.py` (spec §14 open
  question). Check FR-002, FR-004 and FR-005 cite a `fix:` commit where
  they claim `high`.
- [ ] Investigator prompt: *"The most recent change to a file is not
  corroboration. `high` needs a commit the bundle labels `fix`."*
- [ ] **Commit** `validate: high confidence needs a fix commit; report shows commit subjects (AC-5, AC-3)`
...
**Files:** `scripts/report.py` (`render_index`, "no finding" section),
`scripts/guardrail.py` (`build_context`), `tests/test_guardrail.py`,
...
- [ ] **Failing tests:**
  ...
  - the report says H06 has `no finding of its own; cited as evidence by FR-001`;
  - the guardrail latency test still passes.
- [ ] **Implement** per spec §2.4. The guardrail change is one conditional
  in the line format, still stdlib-only.
...
- [ ] **Failing tests** (fixture plus two planted files: `mobile/App.kt` and
  ...
  - `hotspots.json.coverage_gaps` has `unsupported[".kt"] == 1`,
    ...
    `tracked == considered + unchanged + sum(excluded) + sum(unsupported)`;
...
- [ ] **Implement** per spec §2.1–2.2. `coverage_gaps` is computed from the
  `tracked` set `build()` already has. Keep `unsupported` to 8 keys plus
...
uv run --with pytest --with pyyaml --with lizard pytest tests/ -q
uv run scripts/gen_catalog_docs.py --check
uv run scripts/gen_sample_report.py && git diff --exit-code examples/
